...
def is_closed_now():
    now = (datetime.utcnow() + timedelta(hours=3, minutes=30)).time()  # تهران = UTC+3:30
...
async def purge_messages(message: Message):
    ...
    # جمع‌آوری پیام‌ها برای حذف دسته‌ای
    messages_to_delete = []
    async for msg in bot.get_chat_history(message.chat.id, limit=count + 1):
        messages_to_delete.append(msg.message_id)

    # حذف دسته‌ای پیام‌ها (batch)
    deleted_count = 0
    for i in range(0, len(messages_to_delete), 10):  # دسته‌های ۱۰تایی
        ...
        for msg_id in batch:
            try:
                ...
                pass
        await asyncio.sleep(0.2)  # جلوگیری از خطای Flood

    confirm = await message.answer(f"🧹 {deleted_count} پیام پاک شد.")
